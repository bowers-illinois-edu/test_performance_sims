...
## Specify no local additional adjustment
## sim_parms$local_adj_fn <- "local_min_p"

data_path <- file.path(here(), "Simple_Analysis/CSVS_unadj_new")
nsims <- 1000
...
res_unadj <- mclapply(1:nrow(sim_parms), function(i) {
  set.seed(12345) ## same seed for each set of parms
  parms <- sim_parms[i, ]
  message(paste(parms[1, ], collapse = " "))
  ptm <- proc.time()
  res <- simulate_many_runs(n_sim = nsims, k = parms$k, l = parms$l, t = parms$prop_tau_nonzero, local_adjust_fn = local_min_p)
  etm <- proc.time() - ptm
  parms$fwer <- mean(res)
...
save(res_unadj, file = here("Simple_Analysis", "simple_unadj_results.rda"))

res_unadj_dt <- bind_rows(res_unadj)

save(res_unadj_dt, file = here("Simple_Analysis", "res_unadj_dt.rda"))
