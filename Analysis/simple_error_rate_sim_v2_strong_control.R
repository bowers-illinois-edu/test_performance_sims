...
assign_false_H <- function(nodes_per_level, num_levels, prop_false) {
  # Calculate total number of leaves
  num_leaves <- nodes_per_level^(num_levels - 1)

  # Randomly select leaves to have false null hypotheses of no effects (i.e. leaves to have actual treatment effects)
  num_selected <- floor(prop_false * num_leaves)
  selected_leaves <- sample(1:num_leaves, num_selected)

  # Create a logical vector for all nodes
  total_nodes <- (nodes_per_level^num_levels - 1) / (nodes_per_level - 1)
  has_false_hyp <- logical(total_nodes)

  ## TODO START HERE
  lvl_nms <- character(total_nodes)
  lvl_nms[1] <- "lv_0"
  lvl_nms[2:(nodes_per_level + 1)] <- paste("lv", 1:nodes_per_level, sep = "_")
  for (i in seq(3, (num_levels + 1))) {
    lvl_nms[(nodes_per_level + i - 1):(i + nodes_per_level)] <- paste(lvl_nms[i - 1], 1:nodes_per_level, sep = "_")
  }
  lvl_nms

  lvl_nms <- c("lv_0", lvl_nms)
  names(has_false_hyp) <- lvl_nms

  # Mark selected leaves
  ## Among the total nodes, which ones are the leaves and which the parents?
  leaf_start <- total_nodes - num_leaves + 1
  has_false_hyp[leaf_start + selected_leaves - 1] <- TRUE

  # Propagate up the tree
  for (level in (num_levels - 1):1) {
    level_start <- (nodes_per_level^(level - 1) - 1) / (nodes_per_level - 1) + 1
    level_end <- (nodes_per_level^level - 1) / (nodes_per_level - 1)

    for (node in level_start:level_end) {
      # Check if any child has characteristic A
      child_start <- node * nodes_per_level - (nodes_per_level - 1)
      child_end <- node * nodes_per_level
      has_false_hyp[node] <- any(has_false_hyp[child_start:child_end])
    }
  }

  return(has_false_hyp)
}


test1 <- assign_false_H(nodes_per_level = 2, num_levels = 3, prop_false = .5)
